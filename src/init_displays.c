...
int		initialze_display_f(t_printf *list,/* double ,*/ int *len_num, char **str)
{
	int p;
	int len;
	int d_len;
	...
	*str = (list->fl == 1) ? ft_l_ftoa(list->fll, list->precision_space) :
			ft_l_ftoa(list->f, list->precision_space);
	...
	*len_num = len;
	...
	if (list->minus == 1)
			list->width = 0;
	(list->plus == 1 || list->f < 0 || list->fll < 0) ? len++ : 0;
	return (len);
}
